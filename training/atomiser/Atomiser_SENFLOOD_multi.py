...
class Atomiser_Senflood(pl.LightningModule):
    ...
    def __init__(self, *, config, lookup_table):
        ...
        # =====================================================================
        # 7. GRID SPACING CACHE
        # Avoids O(L²) cdist computation on every decode call.
        # Keyed by number of latents — valid because grid shape is deterministic
        # for a given (tpl, sigma_factor, hexagonal) config.
        # =====================================================================
        self._grid_spacing_cache: Dict[int, torch.Tensor] = {}
    ...
    def _init_decoder(self):
        ...
        local_input_dim = (
            self.latent_dim       # selected latent features
            + self.decoder_pe_dim # relative positional encoding (computed at decode time)
            + self.query_dim_recon # query features (spectral + resolution)
        )
    ...
    def _compute_grid_spacing(self, coords: torch.Tensor) -> torch.Tensor:
        """
        Compute median nearest-neighbour distance between latents.

        Cached by number of latents — valid because the grid is deterministic
        for a given (tpl, sigma_factor, hexagonal) configuration, so L uniquely
        identifies the grid shape within a run.

        Uses O(L²) cdist only on cache miss (first call per grid size).
        """
        L = coords.shape[1]
        if L in self._grid_spacing_cache:
            return self._grid_spacing_cache[L]

        with torch.no_grad():
            c     = coords[0]   # [L, 2]
            dists = torch.cdist(c.unsqueeze(0), c.unsqueeze(0)).squeeze(0)  # [L, L]
            dists.fill_diagonal_(float('inf'))
            spacing = dists.min(dim=-1).values.median()

        self._grid_spacing_cache[L] = spacing
        return spacing
    ...
    def _decode_single_grid(self, latents, coords, query_coords, query_gsd,
        ...
        out = self.neighbor_cross_attn(q_flat, kv_flat, dist_bias=None)
