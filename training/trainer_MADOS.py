"""
MADOS Trainer (SKIP variant) for Atomizer
==========================================

Adapted from Model_SenFlood_Skip. Reuses the SAME skip-variant encoder,
Atomiser_Senflood_Skip (decoder pixel-skip cascade) — no MADOS-specific
encoder exists, and none is needed: the encoder is task-agnostic given
num_classes from config.

The skip cascade reads query_token_idx / query_token_valid from the batch
(provided by MADOSDataset + collate_grouped_skip). Those flow straight
through to the encoder, so the loss/label path here is unchanged.

IMPORTANT — sliding window:
  Unlike Sen1Floods11 (512x512, decoded whole), MADOS tiles routinely
  exceed the 240x240 focus-crop size and rely on sliding-window inference
  at val/test (trainer["slide"] = True). MADOSDataset now attaches
  query_token_idx/valid to EACH sliding crop (full pixel grid per crop,
  kept_indices=None), so the skip fields exist per-crop in principle.

  However, _forward_crop below builds its mini_batch by slicing
  batch["queries"] / batch["groups"] per crop and historically dropped any
  other batch keys (see Model_SenFlood_Skip._forward_crop, which builds a
  mini_batch WITHOUT query_token_idx/valid). Whether the MADOS collate
  function (collate_grouped_skip) actually stacks query_token_idx/valid
  per-crop in a way _forward_crop can slice has NOT been verified here.
  To avoid a silent skip-disable under sliding-window eval (same failure
  mode Model_SenFlood_Skip guards against), __init__ still raises if
  use_decoder_skip and slide are both true, UNLESS the person running this
  has confirmed the collate path forwards the per-crop skip fields — in
...
# Reuse the SAME skip-variant encoder used for Sen1Floods11 — task-agnostic.
...
class Model_MADOS_Skip(pl.LightningModule):
    def __init__(self, config, wand, name, transform, lookup_table,
        ...
        # >>> MADOS: same silent-invalidation guard as Model_SenFlood_Skip.
        # MADOS's sliding-window path (_forward_crop below) does not
        # currently forward query_token_idx/valid per crop, which would
        # disable the skip at eval without error. Fail loudly instead,
        # unless/until _forward_crop is extended to slice those fields.
        ...
        if use_decoder_skip and self.use_sliding:
            raise ValueError(
                ...
                "sliding-window path (_forward_crop) does not forward "
                "query_token_idx/valid per crop and would silently disable "
                "the skip cascade at eval time, invalidating the comparison. "
                "Either set trainer.slide=False (use the 240x240 focus-crop "
                "path for eval too), or extend _forward_crop to slice and "
                "pass through the per-crop skip fields that MADOSDataset "
                "now attaches to each sliding crop."
            )
        # >>> END MADOS
        ...
        # MODEL
        # =====================================================================
        # >>> MADOS: same skip-variant encoder as Sen1Floods11, task-agnostic.
    ...
    # =========================================================================
    # NOTE: guarded off when use_decoder_skip is True (see __init__), since
    # this does NOT forward query_token_idx/valid per crop. Kept intact so
    # this trainer remains a drop-in for the non-skip baseline.
    ...
    def _forward_crop(self, batch, crop_idx):
        ...
        # >>> MADOS: if/when the collate function is confirmed to stack
        # query_token_idx/valid per crop, slice and forward them here so
        # sliding-window eval can use the skip cascade too:
        # if "query_token_idx" in batch:
        #     mini_batch["query_token_idx"] = batch["query_token_idx"][crop_idx:crop_idx + 1]
        #     mini_batch["query_token_valid"] = batch["query_token_valid"][crop_idx:crop_idx + 1]
    ...
    def save_model(self, name=None):
        ...
        print(f"[MADOS] Model saved to {file_path}")

    def load_model(self, name=None):
        suffix    = f"_{name}" if name else ""
        file_path = f"./pth_files/{self.config['encoder']}_{self.name}{suffix}.pth"
        self.encoder.load_state_dict(torch.load(file_path, weights_only=True))
        print(f"[MADOS] Model loaded from {file_path}")
