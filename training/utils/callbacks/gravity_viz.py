...
class ErrorLandscapeVisualizationCallback(pl.Callback):
    ...
    def _visualize_sample(self, sample_idx, viz_idx, dataset, pl_module, epoch, split):
        ...
        error_landscape_fig = self._create_error_landscape(
            final_positions, final_errors, rgb_background, sample_idx, epoch, len(predicted_errors)-1
        )
        ...
        gravity_mag_fig = self._create_gravity_magnitude_map(
            final_positions, final_errors, rgb_background, sample_idx, epoch
        )
        ...
        combined_fig = self._create_combined_visualization(
            final_positions, final_errors, all_layer_positions, all_layer_errors,
    ...
    def _compute_continuous_gravity(
        ...
        errors: np.ndarray,     # Discrete errors [L]
    ...
    # ... [Viz functions updated to use _compute_continuous_gravity] ...

    def _create_error_landscape(self, positions, errors, rgb_background, sample_idx, epoch, layer_idx):
        # This remains unchanged (standard interpolation)
        ...
        ax.scatter(positions[:, 0], positions[:, 1], c=errors, cmap='hot', s=50, edgecolors='white',
                   vmin=errors.min(), vmax=errors.max(), zorder=10)
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04).set_label('Pred. Error')
        ax.set_title(f'Error Landscape\nSample {sample_idx}, Epoch {epoch}')
        return fig
    ...
    def _create_combined_visualization(self, positions, errors, trajectory, all_errors, rgb_background, sample_idx, epoch):
        ...
        im1 = ax1.imshow(error_grid_smooth, extent=extent, cmap='hot', alpha=0.7, origin='lower', vmin=errors.min(), vmax=errors.max())
        ax1.scatter(positions[:, 0], positions[:, 1], c=errors, cmap='hot', s=40, edgecolors='white', vmin=errors.min(), vmax=errors.max())
