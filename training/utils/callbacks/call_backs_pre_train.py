...
class PretrainSegVizCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        # All ranks must evaluate this condition. If true, everyone skips and syncs natively.
        if (trainer.current_epoch + 1) % self.log_every_n_epochs != 0:
            return

        # Rank 0 performs the actual visualization
        if trainer.global_rank == 0:
            dataset = _get_multitask_dataset(trainer)
            
            # Guard against missing dataset or disabled task
            if dataset is not None and (not hasattr(dataset, "enabled_tasks") or self.task_name in dataset.enabled_tasks):
                device = pl_module.device
                
                # UNWRAP THE MODEL to bypass DDP forward hooks/collectives
                unwrapped_model = pl_module.module if hasattr(pl_module, "module") else pl_module
                ...
                for idx in self.sample_indices:
                    ...
                    try:
                        ...
                        # Use the unwrapped model for inference
                        with torch.no_grad():
                            all_predictions = unwrapped_model.forward_multitask(
        ...
        # DDP BARRIER: All ranks wait here until Rank 0 finishes plotting
        if torch.distributed.is_initialized():
            trainer.strategy.barrier(f"viz_sync_seg_{self.task_name}")
...
class PretrainReconVizCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        # All ranks skip together if it's not a logging epoch
        if (trainer.current_epoch + 1) % self.log_every_n_epochs != 0:
            return

        # Rank 0 performs visualization
        if trainer.global_rank == 0:
            dataset = _get_multitask_dataset(trainer)
            
            if dataset is not None and (not hasattr(dataset, "enabled_tasks") or "reconstruction" in dataset.enabled_tasks):
                device = pl_module.device
        ...
        # DDP BARRIER: All ranks wait here
        if torch.distributed.is_initialized():
            trainer.strategy.barrier("viz_sync_recon")
