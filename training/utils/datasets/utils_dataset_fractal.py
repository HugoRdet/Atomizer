...
# ============================================================================
# LAS code → FRACTAL 7-class label remap (unchanged from base version)
# ============================================================================
...
class FractalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Initial clip — handles small float excursions
        ...
        # ── Z normalization (ground-relative, unchanged) ───────────
        # D4 augmentation doesn't change z (height is invariant under
        # horizontal rotation), so we compute z_norm before D4. Jitter
        # WILL touch z later (small Gaussian noise).
        ...
        # Same transform as the ortho below — keeps spatial alignment.
        # z unchanged (rotation in the horizontal plane doesn't change
        # height); labels unchanged (class is a point property, not its
        # position).
        ...
        # Seeded from sample index so jitter is reproducible per epoch.
        if self.augmenter.enabled and (self.sigma_xy_pixels > 0
                                       or self.sigma_z_normed > 0):
            # Salted seed so jitter RNG state differs from D4 RNG state.
        ...
        # D4 + jitter can produce values slightly outside [0, P) due to:
        #  - sub-pixel drift in apply_to_xy (P-1 - x with float x)
        #  - Gaussian jitter that pushes a few points across the boundary
        # Clip is mandatory before tokenization, which assumes positions
        # are valid pixel indices in [0, P).
        xy_stacked = np.clip(
        ...
        values_lidar    = torch.from_numpy(z_norm).float()
        labels_lidar    = torch.from_numpy(labels.astype(np.int64))
        ...
        # Dense label for VHR tokens: all IGNORE
        ...
        # ── Build queries: one per LIDAR point ──────────────────────
        ...
        assert queries.shape[0] == values_lidar.shape[0], (
            ...
            f"count ({values_lidar.shape[0]})."
        )
        queries[:, 0] = values_lidar
