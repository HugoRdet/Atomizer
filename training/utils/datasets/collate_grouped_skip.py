...
def collate_grouped_skip(batch: list) -> dict:
    B = len(batch)
    ...
    # ── Build groups (identical to collate_grouped) ─────────
    groups = {}
    for res in sorted(all_resolutions):
        sample_tokens = []
        sample_masks = []
    ...
    # ── Queries + per-query skip index, padded IN LOCKSTEP ──
    query_list = [s["queries"] for s in batch]
    qmask_list = [s["queries_mask"] for s in batch]
    has_skip = "query_token_idx" in batch[0]
    if has_skip:
        idx_list   = [s["query_token_idx"]   for s in batch]   # [M_i, C]
        valid_list = [s["query_token_valid"] for s in batch]   # [M_i]
        C_idx = idx_list[0].shape[1]
    ...
    padded_idx     = []
    padded_valid   = []
    for i, (q, qm) in enumerate(zip(query_list, qmask_list)):
        m = q.shape[0]
        if m < M_max:
            pad_n = M_max - m
            padded_queries.append(torch.cat([q,  torch.zeros(pad_n, 8, dtype=q.dtype)], dim=0))
            padded_qmasks.append( torch.cat([qm, torch.ones(pad_n, dtype=torch.bool)], dim=0))
            if has_skip:
                padded_idx.append(torch.cat(
                    [idx_list[i], torch.zeros(pad_n, C_idx, dtype=idx_list[i].dtype)], dim=0))
                padded_valid.append(torch.cat(
                    [valid_list[i], torch.zeros(pad_n, dtype=torch.bool)], dim=0))
        else:
            padded_queries.append(q)
            padded_qmasks.append(qm)
            if has_skip:
                padded_idx.append(idx_list[i])
                padded_valid.append(valid_list[i])
    ...
    if has_skip:
        result["query_token_idx"]   = torch.stack(padded_idx, dim=0)    # [B, M_max, C]
        result["query_token_valid"] = torch.stack(padded_valid, dim=0)  # [B, M_max] bool

    # ── Optional pass-through keys (same as collate_grouped) ─
    if "image" in batch[0]:
        result["image"] = torch.stack([s["image"] for s in batch], dim=0)

    if "label" in batch[0]:
        result["label"] = torch.stack([s["label"] for s in batch], dim=0)
