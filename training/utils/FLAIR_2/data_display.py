...
def get_sample(idx,images, masks, sentinel_imgs, centroid,sentinel_products,sentinel_masks,aerial_mtd, palette=lut_colors) -> None:
    ...
    sen = np.load(sentinel_imgs[idx])
    sen_mask=np.load(sentinel_masks[idx])
    sen_spatch = sen[:,:, centroid[idx][0]-int(20):centroid[idx][0]+int(20),centroid[idx][1]-int(20):centroid[idx][1]+int(20)]
    
    img_dates = read_dates(sentinel_products[idx])
    tmp_aerial_mtd=aerial_mtd[idx]


    

    return im,mk,sen_spatch,img_dates,sen_mask,tmp_aerial_mtd
...
def compute_channel_mean_std_FLAIR(images, masks, sentinel_imgs, centroid, sentinel_products,sentinel_masks,aer_mtd):
