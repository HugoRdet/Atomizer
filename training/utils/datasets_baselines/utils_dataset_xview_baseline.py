"""
xView2 Damage Assessment — Baseline Dataset (PANGAEA setup)
=============================================================

5-class semantic segmentation matching PANGAEA's xView2.py:
...
Task framing:
    ...
    - Target: post-disaster mask directly, [H, W] in {0..4}
    - No IGNORE gating, no remapping. Background is class 0 in both
      loss and metric. Heavy class imbalance (94% background) is
      handled implicitly by the model rather than via masking.
...
+ random 512×512 crop biased toward regions with buildings (so most
training batches contain useful signal for damage discrimination).
Val/test: deterministic 512×512 center crop with same building bias.
...
Notes on PANGAEA compatibility:
    ...
    - Numbers are directly comparable to PANGAEA's published xView2
      results (5-class mIoU including background).
"""
...
# PANGAEA setup: 5-class semantic segmentation (background + 4 damage levels)
NUM_CLASSES = 5
# IGNORE_INDEX is kept as a sentinel value in case some pixels need masking
# (e.g., if mask reading fails). With PANGAEA's setup, no actual pixels use it
# under normal operation. We use 255 (outside the valid {0..4} range) so the
# trainer's ignore_index handling is safe.
IGNORE_INDEX = 255
...
class XView2BaselineDataset(Dataset):
    ...
    def __init__(
        ...
        self.full_image = full_image  # Return uncropped 1024×1024 (for sliding-window test)
        ...
        print(f"[xView2-BL] num_classes={NUM_CLASSES} (PANGAEA setup: "
              f"0=BG, 1=NoDamage, 2=Minor, 3=Major, 4=Destroyed)")
    ...
    def _crop_prefer_buildings(
        ...
        Under PANGAEA's setup, "building" means any class > 0 in the post
        mask (classes 1-4 = damage levels). Class 0 is background.

        At training (random=True): retry random crops up to max_retries
        times until a crop contains at least one building pixel.
        If all retries fail, fall back to a building-centered crop.

        At val/test (random=False): start with center crop. If it has no
        building pixels, fall back to a building-centered crop.

        This biases training toward informative regions (where the model
        actually has to discriminate damage). Without this, most random
        crops on a typical xView2 tile would be 100% background and
        provide zero training signal for damage classification.
        """
        T, C, H, W = image.shape
        assert H >= crop_size and W >= crop_size
        ...
        def _has_building(lbl):
            # Class > 0 means a building pixel (any damage level).
        ...
        # No building anywhere in the source image — return center crop.
        # Loss is still defined under PANGAEA setup (all background = class 0).
    ...
    def __getitem__(self, idx):
        ...
        # ── Build target: PANGAEA's setup — msk_post directly as 5-class target.
        # msk_post in {0..4}: 0=background, 1=no-damage, 2=minor, 3=major, 4=destroyed
        # No remapping, no IGNORE gating. Background is class 0, included in
        # both loss and metric.
        msk_post = msk_post.astype(np.int64)
        # Defensive: clip any unexpected values (shouldn't happen, but safe)
        target = np.where(
            (msk_post >= 0) & (msk_post < NUM_CLASSES),
            msk_post,
            0,  # treat malformed values as background
        )
        ...
        # else: keep full 1024×1024 for sliding-window evaluation
