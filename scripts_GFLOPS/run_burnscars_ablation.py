"""
BurnScars Density Generalization Driver (TEST-ONLY)
======================================================

Loads ONE trained checkpoint and evaluates it on the test split under all
...
Ported from the Sen1Floods11 density-eval driver. This driver is entirely
task-agnostic (it just shells out and regexes a "RESULT ..." line), so the
port is a rename — EXCEPT for one real dependency, flagged below.

# >>> FLAG: this script calls TEST_SCRIPT = "script_test_burnscars_density_skip.py".
# I have not seen (and have not been given) a Sen1Floods11 equivalent of that
# per-run test script in this conversation, so I don't know what it does
# internally — presumably: load ckpt, override tokens_per_latent/cross_k in
# config, build the BurnScars datamodule, run trainer.test, and print a line
# matching RESULT_RE. If "script_test_senflood_density_skip.py" hardcodes
# anything BurnScars-specific (dataset class, config path, data root, GSD),
# you'll need a BurnScars analogue of THAT script too — this driver alone
# does not create it. Share it if you want me to port it.
...
Usage:
    python run_burnscars_density_eval.py --ckpt ./checkpoints/best.ckpt
    python run_burnscars_density_eval.py --ckpt ... --only 0 5 11
    python run_burnscars_density_eval.py --ckpt ... --dry_run
"""
...
#TABLE1 += [(8000,1000),(16000,1000),(32000,1000)]
TABLE2=[]
TABLE1=[(2000,1000),(4000,1000),(8000,500)]
#TABLE2 = [(4000, ck) for ck in
#          [1,10,50, 100, 300, 500, 700, 900, 1100, 1300, 1500, 1700, 1900, 2000]]  # idx 11..21
#TABLE2=[]
CONFIGS = TABLE1 + TABLE2


# >>> RENAME: BurnScars per-run test script. See FLAG above — this file
# must exist and must itself be BurnScars-aware (dataset/config/data root).
...
for run_n, idx in enumerate(indices, 1):
    ...
    # >>> RENAME: xp prefix
    xp = f"denseval_burnscars_t{table}_tpl{tpl}_ck{ck}_{stamp}"
...
print("\n" + "=" * 78)
print("DENSITY GENERALIZATION — TEST mIoU + GFLOPs (single checkpoint, BurnScars)")
print("=" * 78)
...
# >>> RENAME: csv filename
csv_path = f"training/ablation_runs/density_eval_burnscars_{stamp}.csv"
