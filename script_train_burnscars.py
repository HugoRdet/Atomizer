"""
BurnScars Training Script (SKIP variant) — with --test_only mode

Ported from the Sen1Floods11 skip training script. All changes tagged:
  # >>> RENAME  cosmetic (paths, names, wandb project)
  # >>> CHANGE  behavioral (band count, GSD, dataset/trainer classes)
  # >>> FLAG    unverified — please confirm before running

# >>> FLAG: TokenAssignmentCallbackSenFlood is reused as-is below. There is
# no BurnScars-specific version. I have not seen its source, so I cannot
# confirm it's agnostic to band count (13+2 for Sen1Floods11 vs 6 for
# BurnScars) or to the single-sensor vs fused-sensor token layout. If it
# hardcodes band names/indices or assumes S1+S2 fusion, it will silently
# produce wrong or empty visualizations rather than crash. Worth a quick
# read before trusting its output on the first run.
"""
...
# >>> RENAME: BurnScars trainer + dataset
...
# >>> FLAG: reused as-is, see module docstring
...
# >>> CHANGE: BurnScars config
config_model = read_yaml("./training/configs/config_test-BURNSCARS.yaml")
# >>> unchanged: lookup table is shared across tasks, not task-specific
...
if os.environ.get("LOCAL_RANK", "0") == "0" and args.test_only is None:
    import wandb
    # >>> RENAME: wandb project
    wandb.init(name=config_model["encoder"], project="BurnScars", config=config_model)
...
# >>> FLAG: SenFlood-named callback, see module docstring
...
if os.environ.get("LOCAL_RANK", "0") == "0":
    ...
    for input_size in [512]:
        ...
        results.append({
            # >>> CHANGE: BurnScars GSD is 30m/px, not 10
            "input_size": input_size, "gsd_target": 30,
