...
Load trained baseline checkpoints (UNet / ViT / ResNet) and evaluate
on the test split under different band configurations.
...
-----
    ...
    python script_test_senflood_baseline_modality_drop.py \
        --ckpt ./checkpoints/bl_unet.ckpt \
        --model unet \
        --xp_name unet_drop_eval

    # Multiple checkpoints in one run (one row per model in the summary table)
    python script_test_senflood_baseline_modality_drop.py \
        --ckpts unet=./checkpoints/bl_unet.ckpt resnet=./checkpoints/bl_resnet.ckpt \
...
# S2: B01=0, B02=1, B03=2, B04=3, B05=4, B06=5, B07=6,
#     B08=7, B08A=8, B09=9, B10=10, B11=11, B12=12
# S1: VV=13, VH=14
BAND_TO_CHANNEL = {
...
class ChannelDropWrapper(nn.Module):
    ...
    This is the baseline equivalent of the Atomiser padding-token mask.
    The model sees the same number of channels (15) — just zeroed for
    dropped bands — so no architectural change is needed.

    Parameters
    ----------
    model        : the underlying segmentation model
    drop_channels: list of int  channel indices to zero (0-based into [B,C,H,W])
    """
...
# =============================================================================
# MODEL BUILDER  (mirrors training script exactly)
# =============================================================================
...
# Architecture args (only needed if they differ from defaults)
...
# =============================================================================
# TEST DATASET  (shared across all runs — no channel selection at dataset level)
# =============================================================================
...
for model_name, ckpt_path in model_ckpts:
    ...
    for ablation_name in args.ablations:
        ...
        # Swap the inner model for a channel-zeroing wrapper
        # BaselineTrainer.forward calls self.model(image) so we replace it.
    ...
    # Restore original model (in case of further loops)
...
# Also print in the same layout as the Atomiser table for easy copy-paste
