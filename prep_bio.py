...
def repair_and_extract_multipart_zip(root: Path, zip_stem: str, dest: Path):
    ...
    file_list = subprocess.run(
        ["unzip", "-Z1", str(fixed_zip)], capture_output=True, text=True, check=True
    ).stdout.splitlines()
    with zipfile.ZipFile(fixed_zip) as zf:
        for name in tqdm(file_list, desc=f"Extracting {zip_stem}"):
...
def extract_single_zip(root: Path, zip_stem: str, dest: Path):
    ...
    with zipfile.ZipFile(zip_path) as zf:
        for name in tqdm(zf.namelist(), desc=f"Extracting {zip_stem}"):
            zf.extract(name, path=str(dest))
    marker.write_text("ok")
    print(f"[ok] extracted {zip_stem} -> {dest}")
...
def extract_tar(root: Path, tar_stem: str, dest: Path):
    ...
    with tarfile.open(tar_path) as tf:
        ...
        for member in tqdm(members, desc=f"Extracting {tar_stem}"):
    ...
    for split, chips in manifests.items():
        ...
        for chip_id, sats in chips.items():
            records.append({
                ...
                "agbm_file": agbm_lookup.get(chip_id) if split == "train" else None,
            })
