"""
MADOS Perceiver-IO Single-Task Training Script
=================================================

Trains the Perceiver-IO baseline on MADOS — 15-class marine debris
segmentation on Sentinel-2, single-frame.

Token layout (matches PerceiverSeg.forward):
    Per-token feature = [reflectance(C), Fourier(x, y), no_time_vector]
...
in the time slot for both tokens and queries (cosmetic — adds no real
temporal information, but keeps the token-feature dim consistent with
the multi-temporal PASTIS run).

Examples:
    # Default config (matches Atomizer parameter scale)
    python script_train_mados_perceiver.py --xp_name perceiver_mados \
        --batch_size 4 --lr 1e-4 --epochs 80

    # Test-only mode
    python script_train_mados_perceiver.py --xp_name perceiver_mados \
        --test_only ./checkpoints/mados_perceiver/bl_perceiver_mados-best.ckpt
"""
...
parser = argparse.ArgumentParser(description="MADOS Perceiver-IO Baseline")
...
# Spatial — MADOS native is 240x240
parser.add_argument("--img_size", type=int, default=NATIVE_H,
                    help=f"Spatial size. Default {NATIVE_H} (full MADOS patch).")

# Perceiver-IO config (matches the PASTIS run for parameter parity)
...
if "bands_mados" not in bands_yaml:
    raise KeyError(
        f"[MADOS-Perceiver] bands_yaml ({args.bands_yaml}) must contain a "
...
print(f"\n{'='*60}")
print(f"  MADOS Perceiver-IO Baseline")
...
ckpt_dir = "./checkpoints/mados_perceiver/"
...
if args.test_only is None:
    ...
    print(f"\n{'='*60}")
    print(f"  Starting: perceiver on MADOS")
    print(f"{'='*60}\n")
